...
#define KT_TOAST_PADDING_HEIGHT     14
#define KT_TOAST_PADDING_WIDTH      17
#define KT_TOAST_MAX_WIDTH          220
#define KT_TOAST_CORNER_RADIUS      5.0
...
//    self.layer.shadowOpacity = 0.8f;
    ...
    newLabelFrame.origin = CGPointMake(KT_TOAST_PADDING_WIDTH, KT_TOAST_PADDING_HEIGHT); // have an offset for padding
    toastLabel.frame = newLabelFrame;
    
    // add the toast label to the alert (centered with padding)
    CGRect newViewFrame = self.frame;
    newViewFrame.size.width = newLabelFrame.size.width + KT_TOAST_PADDING_WIDTH * 2;
    newViewFrame.size.height = newLabelFrame.size.height + KT_TOAST_PADDING_HEIGHT * 2;
...
//    self.frame = CGRectMake(round(self.frame.origin.x), round(self.frame.origin.y), self.frame.size.width, self.frame.size.height);
    self.frame = CGRectMakeWithCenterAndSize(CGRectCenter(windowFrame), self.frame.size);
}
